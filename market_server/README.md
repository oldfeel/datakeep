...
CREATE ROLE mydata_market LOGIN PASSWORD '应用专用密码';
CREATE DATABASE mydata_market OWNER mydata_market;
GRANT ALL PRIVILEGES ON DATABASE mydata_market TO mydata_market;
\c mydata_market
GRANT ALL ON SCHEMA public TO mydata_market;
ALTER SCHEMA public OWNER TO mydata_market;
SQL
```

配置：复制 `.env.example` 为 `.env`，`DATABASE_URL` 使用 `user=mydata_market`。然后：
