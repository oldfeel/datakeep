...
MyData 是一个基于 Flutter 开发的跨平台文件同步应用，支持桌面端、移动端和 Web 端。
...
- 📱 **设备管理**: 管理多设备同步
- 🔄 **实时同步**: 监控文件变化并自动同步
- 🌐 **跨平台支持**: 桌面（Linux/Windows/macOS）与 Android 为当前主力；iOS 为脚手架（Syncthing 引擎参考 sushitrain，gomobile 待编入）
- 🔗 **互联网分享**: 上传到 S3 兼容存储（推荐七牛）并生成临时下载链接，本机无需公网 IP
