...
func getSyncthingExecutablePath() string {
	// 优先使用项目目录下的 syncthing 可执行文件
	projectRoot := getProjectRoot()
	syncthingPath := filepath.Join(projectRoot, "syncthing", "bin", "syncthing")
	...
	// 如果都找不到，返回项目目录下的路径（即使不存在，让用户知道需要编译）
	return syncthingPath
}

// getProjectRoot 获取项目根目录
func getProjectRoot() string {
	// 从当前工作目录向上查找，直到找到包含 syncthing 目录的目录
	wd, err := os.Getwd()
	if err != nil {
		wd = "."
	}
	...
	dir := wd
	for {
		syncthingDir := filepath.Join(dir, "syncthing")
		if info, err := os.Stat(syncthingDir); err == nil && info.IsDir() {
			return dir
		}
