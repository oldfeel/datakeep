...
#   ./scripts/release.sh download         # 下载四端包到 dist-release/（默认最新 tag）
...
for arg in "$@"; do
  case "$arg" in
    ...
    links|urls)
      ACTION="links"
      ;;
    download|fetch|dist-release)
      ACTION="download"
      ;;
    v[0-9]*.[0-9]*.[0-9]*)
      if [[ "$ACTION" == "market" || "$ACTION" == "links" || "$ACTION" == "download" ]]; then
        RELEASE_TAG="$arg"
      else
...
cmd_download() {
  ...
  if [[ "$SKIP_PGYER" -eq 0 && -f "$apk" ]]; then
    ...
    if load_pgyer_api_key; then
      upload_apk_to_pgyer "$apk" "$tag" || warn "蒲公英上传失败（本地下载不受影响）"
    else
