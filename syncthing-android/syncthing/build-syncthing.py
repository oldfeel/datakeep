...
def get_ndk_home():
    if not os.environ.get("ANDROID_NDK_HOME", ""):
        if not os.environ.get("NDK_VERSION", "") or not os.environ.get(
            "ANDROID_HOME", ""
        ):
            fail(
        ...
        return os.path.join(
            os.environ["ANDROID_HOME"], "ndk", os.environ["NDK_VERSION"]
        )
...
syncthing_dir = os.path.join(module_dir, "../../syncthing")
...
for target in BUILD_TARGETS:
    ...
    subprocess.check_call(
        ["go", "run", "build.go", "version"], env=environ, cwd=syncthing_dir
    )
    cc = os.path.join(
        get_ndk_home(),
        "toolchains",
    ...
    subprocess.check_call(
        [
            ...
            cc,
            "-pkgdir",
            os.path.join(go_build_dir, target["goarch"]),
            "-no-upgrade",
            "build",
        ],
        env=environ,
    ...
    target_dir = os.path.join(
        project_dir, "app", "src", "main", "jniLibs", target["jni_dir"]
    )
